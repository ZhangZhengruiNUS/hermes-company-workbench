...
export function TaskDetailDrawer({ taskId, data, open, onClose, onOpenTask }: Props) {
  ...
  return (
    <DrawerShell open={open} onClose={onClose} label={`任务详情: ${t.title}`}>
      {/* header */}
                    ...
                    <div>
                      ...
                      {detail.runs.map((r, i) => (
                        ...
                        >
                          ...
                          {/* 完整摘要换行展示, 不硬截断 (UI Quality Gate) */}
                          {r.summary && <div className="text-[11px] break-words" style={{ color: "var(--text-3)" }}>{r.summary}</div>}
                        </div>
                      ))}
                    </div>
                  </>
...
function DepRow({ x, mark, onOpenTask }: { x: { id: string; title: string; status: string }; mark: string; onOpenTask: (id: string) => void; }) {
  const full = x.title || x.id;
  return (
    <div className="flex items-center gap-1.5 py-1">
      <span className="text-[11px] shrink-0" style={{ color: "var(--text-3)" }}>{mark}</span>
      {/* 容器 min-w-0 + CSS truncate + title 保留全文, 不硬截断 (UI Quality Gate) */}
      <button
        onClick={() => onOpenTask(x.id)}
        className="flex-1 min-w-0 text-left text-[12px] truncate border-0 p-0 cursor-pointer transition-colors"
        style={{ color: "var(--accent-blue)", background: "none" }}
        title={full}
      >
        {full}
        <ExternalLink size={10} className="inline ml-0.5 shrink-0" style={{ color: "var(--text-3)" }} />
      </button>
      <span className="text-[10px] ml-auto shrink-0" style={{ color: "var(--text-3)" }}>{x.status}</span>
    </div>
  );
}